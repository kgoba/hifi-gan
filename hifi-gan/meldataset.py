...
def mel_spectrogram(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax):
    ...
    if key not in mel_basis:
        ...
        hann_window[key] = torch.hann_window(win_size).to("cpu") / win_size

    y = torch.nn.functional.pad(y.unsqueeze(1), (int((n_fft-hop_size)/2), int((n_fft-hop_size)/2)), mode='constant')
    y = y.squeeze(1)
    ...
    spec = (spec.real**2 + spec.imag**2) * 100
